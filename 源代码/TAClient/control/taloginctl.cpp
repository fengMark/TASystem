...
TALoginCtl::TALoginCtl(QObject *parent) :
    QObject(parent)
{
...
*************************************************/
void TALoginCtl::login(const QString &id, const QString &pwd, const int status)
{
    m_kind = LOGIN;
    m_loginInfo.TA_userID= id;
    m_loginInfo.TA_password = pwd;
    m_loginInfo.TA_status = status;
    if (m_tcpSocket->isConnected())
    {
        requestLogin();
    }
...
Description: 获取密保信息
*************************************************/
void TALoginCtl::getQuestionAndAnswer(const QString & id)
{
...
Changes： NULL
*************************************************/
void TALoginCtl::changeLoginMessage(const QString &mes, bool isLogin)
{
...
*************************************************/
void TALoginCtl::requestGetQuestionAndAnswer()
{
    if (NULL == m_tcpSocket)
        return;
    m_blockSize = 0;
...
void TALoginCtl::readMessage()
{
    ...
    {
        in >> m_tempStr;//读取密码以及密保问题和答案
        emit getQuestionAndAnswerSuccess(m_tempStr);

        while (1)
        {
            bool isOkMes = false;
            QString str = QString(tr("密保问题:%1\n请输入问题答案:")).arg(m_tempStr.TA_two);
            QString answer = QInputDialog::getText(NULL, "找回密码", str, QLineEdit::Normal,NULL,&isOkMes);

            if (answer != m_tempStr.TA_three)
            {
                str = QString(tr("回答错误!"));
            ...
            {
                str = QString(tr("回答正确!\n您的帐号是:%1\n您的密码是:%2")).arg(m_id).arg(TAEncryption::getInstace().getXorEncryptDecrypt(m_tempStr.TA_one, 10));
