...
#include<QDataStream>
class TAUser : public QObject
{
    Q_OBJECT
public:
    TAUser(QObject *parent = 0);
    TAUser(QString id, const QString &nick, const QString &pwd = "",
           const QString &sex = "",
...
private:
    ...
    friend QDataStream &operator<<(QDataStream &out, const TAUser &user);

    friend QDataStream &operator>>(QDataStream &in, TAUser &user);
};
